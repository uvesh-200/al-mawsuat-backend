"""Unit tests: citation resolution fixes for printed page numbers and the
dense-page anchoring (BUG 1 / Bug 1.3 regression guards).

1. Multi-page passages carrying per-offset ``printed_page_num`` show ⟨Page N⟩
   markers and Source lines in PRINTED numbering; physical numbers remain the
   fallback for books without footers.
2. ``_claim_page`` ranks passages by matched-token count first (an anchor-only
   match must not beat a long exact match) and anchors on the page that
   contains MOST matched tokens (dense page), not the last matched token —
   claims ending in high-frequency words drift onto later pages otherwise.
"""
import pytest
...
def _passage(
    ...
    return {
        ...
        "page_end": page_end,
        "printed_page_start": page_start + 6 if pages else None,
        "printed_page_end": page_end + 6 if pages else None,
        "chapter": None,
        "page_offsets": [
            {
                "page": i + page_start,
                "start_char": start,
                "end_char": (offs[i + 1] if i + 1 < len(offs) else len(text)) - 1,
                "printed_page_num": (i + page_start + 6) if pages else None,
            }
            for i, start in enumerate(offs)
        ]
        if (offs := [p["start_char"] for p in pages])
        else (
            [{"page": page_start, "start_char": 0, "end_char": len(text) - 1, "printed_page_num": None}]
            if not pages
            else []
        ),
    }
...
class TestPrintedMarkers:
    def test_marker_shows_printed_page_when_known(self):
        offsets = [
            {"page": 5, "start_char": 0, "end_char": 10, "printed_page_num": 11},
            {"page": 6, "start_char": 11, "end_char": 20, "printed_page_num": 12},
        ]
        annotated = _annotate_pages("a mid one b", [])
        annotated = _annotate_pages("aaaaaaaaaa bbbbbbbbbb", offsets)
        assert "\u27e8Page 11\u27e9" in annotated
        assert "\u27e8Page 12\u27e9" in annotated
        assert "\u27e8Page 5\u27e9" not in annotated

    def test_annotate_falls_back_to_physical(self):
        offsets = [
            {"page": 5, "start_char": 0, "end_char": 10},
            {"page": 6, "start_char": 11, "end_char": 20},
        ]
        annotated = _annotate_pages("aaaaaaaaaa bbbbbbbbbb", offsets)
        assert "\u27e8Page 5\u27e9" in annotated
        assert "\u27e8Page 6\u27e9" in annotated

    def test_format_passages_printed_range(self):
        offsets = [
            {"page": 5, "start_char": 0, "end_char": 10, "printed_page_num": 11},
            {"page": 6, "start_char": 11, "end_char": 20, "printed_page_num": 12},
        ]
        ...
        assert "Page 11-12" in formatted
        assert "\u27e8Page 11\u27e9" in formatted
...
class TestClaimPageScoring:
        ...
        msg = [
            {
                ...
                "chapter": None,
                "page_offsets": [
                    {"page": 1, "start_char": 0, "end_char": len(wrong_text) - 1, "printed_page_num": 7},
                ],
            },
            {
                "book_name": "How to Love Prophet",
                ...
                "chapter": None,
                "page_offsets": [
                    {"page": 5, "start_char": 0, "end_char": len(cited_text) - 1, "printed_page_num": 11},
                ],
            },
        ]
        return msg
    ...
    def test_long_exact_match_beats_single_anchor(self):
        ...
        # the long claim lives on printed page 11 (chunk start)
        assert page == 11

    def test_tiny_generic_claim_cannot_hijack_page(self):
        """Regression: a 2-token generic lead-in ("according to") that also
        occurs in an unrelated chunk (printed 14) must not drag the cited
        chunk's citation away from its own pages (printed 8); the page falls
        back to the cited chunk's printed start."""
        ...
        cited = {
            ...
            "chapter": None,
            "page_offsets": [
                {"page": 2, "start_char": 0, "end_char": len(cited_text) - 1, "printed_page_num": 8},
            ],
        }
        other = {
            "book_name": "How to Love Prophet",
            ...
            "chapter": None,
            "page_offsets": [
                {"page": 7, "start_char": 0, "end_char": len(other_text) - 1, "printed_page_num": 14},
            ],
        }
        # the model tagged [P1, Page 14]; the claim "according to" is generic
        assert _claim_page([cited, other], 0, "14", ["according", "to"]) == 8

    def test_citation_resolution_uses_printed_page(self):
        """[P1, Page 5] on a chunk whose pages print as 12..14 resolves to
        the printed number via the offset's printed_page_num."""
        ...
        p = {
            ...
            "chapter": None,
            "page_offsets": [
                {"page": 5, "start_char": 0, "end_char": 20, "printed_page_num": 12},
                {"page": 6, "start_char": 21, "end_char": len(text) - 1, "printed_page_num": 13},
            ],
        }
        # claim text "one narration is from" lives in the page-5 slice
        resolved = _resolve_citations(
            "هذا القول ورد [P1, Page 12]. واحد آخر [P1, Page 13].",
            [p],
        )
        assert "[How to Love Prophet, Page 12]" in resolved
        assert "[How to Love Prophet, Page 13]" in resolved
